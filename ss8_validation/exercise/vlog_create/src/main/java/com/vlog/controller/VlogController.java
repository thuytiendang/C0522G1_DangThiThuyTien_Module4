...
public class VlogController {
    ...
    @GetMapping("/")
    public String showList(@PageableDefault(value = 3, sort = "created_date", direction = Sort.Direction.ASC) Pageable pageable,
                         @RequestParam(defaultValue = "") String title, Model model) {
        model.addAttribute("vlogList", iVlogService.findByTitle(title,pageable));
